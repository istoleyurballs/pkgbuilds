...
def "main install" [
  --esp: path = "/boot",
  --disk: path = "/dev/disk/by-partlabel/coruscant-boot",
  --partition: int = 1,
  --root: path = "/"
] {
  if not (is-admin) {
  ...
  (^efibootmgr --create
    ...
    --label 'Arch Linux Coruscant (fallback)' --loader '\EFI\Linux\arch-linux-fallback.efi' --unicode)

  log $"Adding a boot entry for (ansi wb)arch-linux.efi(ansi reset)"
  (^efibootmgr --create
    --disk $disk --part $partition
    --label 'Arch Linux Coruscant' --loader '\EFI\Linux\arch-linux.efi' --unicode)
